                            ...
                            if idthing:  # home, so look in the home row
                                ...
                                if entry["vis_line"][label] == word:
                                    ...
                                    src_idx = (2 * NUM_PLAYERS + 1) * (len(bs_keys) - 1) + \
                                        len(bs_keys) - len(ls_keys) + col_idx - 1
                            if src_idx is not None:
...
def save_coref_task_data(outfile, inp_file="full_newnba_prepdata2.json"):
    ...
    trwindows = [[vocab[wrd] if wrd in vocab else vocab["UNK"] for wrd in window] + [-1] * (max_trlen - len(window)) for (window, label) in datasets[0]]
    trlabels = [label for (window, label) in datasets[0]]
    valwindows = [[vocab[wrd] if wrd in vocab else vocab["UNK"] for wrd in window] + [-1] * (max_vallen - len(window)) for (window, label) in datasets[1]]
